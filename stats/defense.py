...
pa = plays.loc[plays['player'].shift() != plays['player'], ['year', 'game_id', 'team', 'player']]
...
events.columns = ['year', 'game_id', 'team', 'BB', 'E', 'H', 'HBC', 'HR', 'ROE', 'SO']
events = events.rename_axis(None, axis='columns')
events_plus_pa = pd.merge(events, pa, how='outer', letft_on=['year', 'game_id', 'team'], righ_on=['year', 'game_id', 'team'])
defense = pd.merge(events_plus_pa, info)
defense.loc[:, 'DER'] = 1 - ((defense['H'] + defense['ROE']) / (defense['PA'] - defense['BB'] - defense['SO'] - defense['HBC'] - defense['HR']))
